# -*- coding: utf-8 -*-
"""One-shot publication of the Larnthong Villa 1BR Bophut listing to @samuirental.

The module is inert unless explicitly called by the existing one-shot launcher.
It determines the lot number from the live channel and verifies the publication
before reporting success.
"""
from __future__ import annotations

import asyncio
import json
import logging
import os
import tempfile
import zipfile
from pathlib import Path

import requests
from google.auth.transport.requests import Request as GoogleAuthRequest
from google.oauth2 import service_account

import cozy_catalog
import mtproto_user_client
import publication_safety

log = logging.getLogger("publish-bophut-larnthong-20260914")

SOURCE_ID = "bophut_larnthong_20260914"
CHANNEL = "samuirental"
BOT_USERNAME = "Cozyasia_villa_bot"
DRIVE_FILE_ID = "1uXnjWNah4EHORoOKVwTf_KoeEPKK-Ew3"
PHOTO_SUFFIXES = (
    "17695.jpg",
    "17701.jpg",
    "17710.jpg",
    "17713.jpg",
    "17716.jpg",
    "17719.jpg",
    "17722.jpg",
    "17725.jpg",
    "17698.jpg",
    "17704.jpg",
)


def _caption_html(lot: str) -> str:
    return f"""🏡 <b>ЛОТ №{lot}</b>

💬 <b>ОПИСАНИЕ</b>
<blockquote>Новый современный дом с 1 спальней в новом резорте Larnthong Villa, Бопхут, рядом с Fisherman's Village. Светлый интерьер, отдельная спальня, гостиная и оборудованная кухня. Общий бассейн, до пляжа около 100 м.</blockquote>

📍 Район: Бопхут / Fisherman's Village
🏠 Тип: дом
🛏 Спальни: 1
🛁 Ванные: 1
🏊 Бассейн: общий
❄️ 2 кондиционера
🌐 Wi-Fi 1000/1000 Mbps
🧹 Уборка: 1 раз в неделю включена

💰 <b>УСЛОВИЯ АРЕНДЫ</b>
💵 Цена: 45 000 THB/мес
🔐 Депозит: 45 000 THB
🤝 Комиссия: 5 000 THB
📅 Доступность: сентябрь — январь
📆 Срок аренды: от 1 месяца
⚡ Электричество: 8 THB/юнит

🗺 <a href="https://share.google/SDL3HiTsk03NkkKSV"><b>ГЕОЛОКАЦИЯ</b></a>

📝 <b>ОСТАВИТЬ ЗАЯВКУ</b>
👉 <a href="https://t.me/{BOT_USERNAME}?start=rent_{lot}"><b>ЖМИ ЗДЕСЬ</b></a> 👈

Оператор: @cozy_asia

🔎🏡 ПОДОБРАТЬ ДРУГИЕ ВАРИАНТЫ — <a href="https://t.me/{BOT_USERNAME}?start=search"><b>НАПИСАТЬ БОТУ</b></a> 🤖

#АрендаСамуи #Бопхут #FishermansVillage #ДомСамуи #KohSamuiRental #CozyAsia"""


def _final_caption(lot: str):
    from telethon.extensions import html as telethon_html

    text, entities = telethon_html.parse(_caption_html(lot))
    text, entities, changed = mtproto_user_client.upgrade_text(text, entities, lot)
    if not changed:
        raise RuntimeError("Premium conversion failed")
    publication_safety.validate_premium_caption(text, entities, lot)
    if len(text) > 1024:
        raise RuntimeError(f"Album caption too long: {len(text)}")
    return text, entities


def _download_photos(directory: str) -> list[str]:
    raw = os.environ.get("GOOGLE_CREDS_JSON", "").strip()
    if not raw:
        raise RuntimeError("GOOGLE_CREDS_JSON is missing")
    info = json.loads(raw)
    creds = service_account.Credentials.from_service_account_info(
        info,
        scopes=["https://www.googleapis.com/auth/drive.readonly"],
    )
    creds.refresh(GoogleAuthRequest())
    response = requests.get(
        f"https://www.googleapis.com/drive/v3/files/{DRIVE_FILE_ID}?alt=media",
        headers={"Authorization": f"Bearer {creds.token}"},
        timeout=90,
    )
    response.raise_for_status()

    zip_path = Path(directory) / "bophut_larnthong_20260914.zip"
    zip_path.write_bytes(response.content)
    photo_dir = Path(directory) / "photos"
    photo_dir.mkdir(parents=True, exist_ok=True)

    paths: list[str] = []
    with zipfile.ZipFile(zip_path) as archive:
        names = archive.namelist()
        for suffix in PHOTO_SUFFIXES:
            matches = [n for n in names if n == suffix or n.endswith("/" + suffix)]
            if len(matches) != 1:
                raise RuntimeError(
                    f"Expected one archive member for {suffix}, found {len(matches)}"
                )
            target = photo_dir / suffix
            target.write_bytes(archive.read(matches[0]))
            paths.append(str(target))
    return paths


async def run() -> dict:
    client = await mtproto_user_client._new_client(cozy_catalog)
    if not client:
        raise RuntimeError("MTProto Premium session is not authorized")
    try:
        channel = await client.get_entity(CHANNEL)

        # Idempotency: if this exact listing was already published, do not duplicate it.
        duplicate = await publication_safety.find_duplicate_listing(
            client,
            channel,
            ("45 000", "Larnthong Villa", "1000/1000"),
            limit=300,
        )
        if duplicate:
            lot = publication_safety.lot_from_message(duplicate)
            result = {
                "channel": CHANNEL,
                "lot": lot,
                "message_id": int(duplicate.id),
                "result": "already",
                "source": SOURCE_ID,
            }
            log.info(
                "PUBLISH_BOPHUT_LARNTHONG_DONE %s",
                json.dumps(result, ensure_ascii=False),
            )
            return {"enabled": True, "result": result}

        # Never guess the number from Sheets or source data: derive it from Telegram live state.
        previous = await publication_safety.latest_numeric_lot(client, channel, limit=300)
        if not previous:
            raise RuntimeError("Could not determine previous live lot")
        lot = str(int(previous) + 1)
        await publication_safety.assert_next_lot(client, channel, lot)
        text, entities = _final_caption(lot)

        with tempfile.TemporaryDirectory(prefix="bophut-larnthong-") as directory:
            photos = await asyncio.to_thread(_download_photos, directory)
            sent = await client.send_file(
                channel,
                photos,
                caption=text,
                formatting_entities=entities,
                link_preview=False,
            )

        messages = sent if isinstance(sent, list) else [sent]
        caption_msg = next(
            (m for m in messages if getattr(m, "message", None)),
            messages[0],
        )
        verify = await client.get_messages(channel, ids=int(caption_msg.id))
        if publication_safety.lot_from_message(verify) != lot:
            raise RuntimeError("Read-back lot mismatch")
        publication_safety.validate_premium_caption(verify.message, verify.entities, lot)
        for signature in (
            "45 000",
            "5 000",
            "Larnthong Villa",
            "сентябрь — январь",
            "8 THB/юнит",
        ):
            if signature not in (verify.message or ""):
                raise RuntimeError(f"Read-back signature missing: {signature}")
        if (verify.message or "").count("НАПИСАТЬ БОТУ") != 1:
            raise RuntimeError("НАПИСАТЬ БОТУ must occur exactly once")

        result = {
            "channel": CHANNEL,
            "lot": lot,
            "message_id": int(caption_msg.id),
            "result": "published",
            "photos": len(PHOTO_SUFFIXES),
            "source": SOURCE_ID,
        }
        log.info(
            "PUBLISH_BOPHUT_LARNTHONG_DONE %s",
            json.dumps(result, ensure_ascii=False),
        )
        return {"enabled": True, "result": result}
    finally:
        await client.disconnect()
